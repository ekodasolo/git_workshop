...
たとえば、自分とAさんが一緒にAND BURGERのサイトを作っているとします。

- **自分：** バーガーメニュー（burgers.md）を編集している
- **Aさん：** サイドメニュー（sides.md）を新しく作成している

2人は別々のファイルを担当しているので、作業は同時に進められます。しかし、それぞれのPCにしかファイルがなければ、お互いの成果を合わせる手段がありません。

ここで登場するのがリモートリポジトリです。サーバー上に共有の拠点を置き、各自がそこに変更を送る（プッシュ）ことで、チーム全員の作業を1つにまとめられます。

```
自分のPC                リモートリポジトリ               AさんのPC
(burgers.md を編集)     (サーバー上の共有拠点)          (sides.md を作成)
      |                        |                         |
      |----  push  ----------->|                         |
      |                        |<----------  push  ------|
      |                        |                         |
...
ここまでの操作はすべて自分のPC（ローカル）の中で完結していました。リモートリポジトリを使うことで、チームメンバーとの共有やバックアップが可能になります。
